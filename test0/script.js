    ...
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', async () => {
            try {
                ...
                console.log('🚀 Drawerz SW: Registered successfully', registration.scope);
                
                // Listen for SW messages
                navigator.serviceWorker.addEventListener('message', (event) => {
                    const { data } = event;
                    if (data.type === 'SW_ACTIVATED') {
                        console.log(`✅ Drawerz SW: Activated v${data.version} at ${new Date(data.timestamp).toLocaleTimeString()}`);
                    }
                });
                ...
                registration.addEventListener('updatefound', () => {
                    ...
                    newWorker.addEventListener('statechange', () => {
                        if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                            console.log('🔄 Drawerz SW: New version available');
                ...
                console.error('❌ Drawerz SW: Registration failed:', error);
            }
        });
    } else {
